...
export const useAccountManager = () => {
  ...
  const enableSignless = useCallback(async (
    ...
    try {
      ...
      const response = await lensRequest(ENABLE_SIGNLESS_MUTATION, {}, accessToken);
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to enable signless experience';
      setState(prev => ({
        ...prev,
        loading: false,
        error: errorMessage,
  ...
  const removeSignless = useCallback(async (
    accessToken: string
  ): Promise<boolean> => {
    setState(prev => ({ ...prev, loading: true, error: null, success: null }));
    ...
    try {
      ...
      const response = await lensRequest(REMOVE_SIGNLESS_MUTATION, {}, accessToken);
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to remove signless experience';
      setState(prev => ({
        ...prev,
        loading: false,
        error: errorMessage,
  ...
  const fetchAccountManagers = useCallback(async (
    ...
    setState(prev => ({ ...prev, managersLoading: true, managersError: null }));

    try {
      console.log('Fetching account managers with request:', request);
      
      const response = await lensRequest(FETCH_ACCOUNT_MANAGERS_QUERY, { request }, accessToken);
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to fetch account managers';
      setState(prev => ({
        ...prev,
        managersLoading: false,
  ...
  const hideManagedAccount = useCallback(async (
    ...
    try {
      ...
      const response = await lensRequest(HIDE_MANAGED_ACCOUNT_MUTATION, {
        request,
      }, accessToken);
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to hide managed account';
      setState(prev => ({
        ...prev,
        loading: false,
        error: errorMessage,
  ...
  const unhideManagedAccount = useCallback(async (
    ...
    try {
      ...
      const response = await lensRequest(UNHIDE_MANAGED_ACCOUNT_MUTATION, {
        request,
      }, accessToken);
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to unhide managed account';
      setState(prev => ({
        ...prev,
        loading: false,
        error: errorMessage,
