...
export const useLensProfiles = (address: string | null) => {
  ...
  const convertToLensProfile = (
    accountItem: AccountManaged | AccountOwned,
    ...
    return {
      ...
      accountManagerPermissions: isManagedAccount
        ? (accountItem as AccountManaged).permissions
        : undefined,
      isManagedAccount,
  ...
  const fetchProfiles = useCallback(async (walletAddress: string) => {
    ...
    try {
      const data: AccountsAvailableResponse = await lensRequest(
      ...
      const convertedProfiles =
        data.value?.items?.map((item) =>
          convertToLensProfile(item, walletAddress),
        ) || [];
      setProfiles(convertedProfiles);
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Failed to fetch Lens profiles",
      );
      setProfiles([]);
    } finally {
      setLoading(false);
