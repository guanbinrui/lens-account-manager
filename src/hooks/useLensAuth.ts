...
import { LensProfile, LensAuthState, AuthenticationChallenge, AuthenticationResult, AuthenticationTokens, ChallengeRequest, SignedAuthChallenge } from '@/types/lens';
import { lensRequest, CHALLENGE_QUERY, AUTHENTICATE_MUTATION } from '@/lib/lens';
...
export const useLensAuth = () => {
  ...
  const signInWithLens = useCallback(async (
    ...
    try {
      console.log('Starting Lens authentication for profile:', profile.id, 'with wallet:', walletAddress);
      ...
      if (result.__typename === 'AuthenticationTokens') {
        ...
        const errorMessage = result.__typename === 'WrongSignerError' 
